...
foreach ($f in $files) {
    $p = "C:\DirectxGame\$f"
    if (Test-Path $p) {
        [System.IO.File]::WriteAllText($p, [System.IO.File]::ReadAllText($p, [System.Text.Encoding]::GetEncoding('shift_jis')), [System.Text.Encoding]::UTF8)
    }
}
Write-Host "All files converted to UTF-8"
